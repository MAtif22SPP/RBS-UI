...
const handlePermission = (result, t) => {
  ...
  switch (result) {
    ...
    case RESULTS.BLOCKED:
      Alert.alert(
        t('allTexts.permissionReq'),
        t('allTexts.allowpermissionReq'),
        [
          { text: t('buttonsTxt.cancel'), style: 'cancel' },
          {
            text: t('allTexts.openSettings'),
            onPress: () => {
...
const permissionUtils = {
  getSinglePermission: async (permission, t) => {
    let isPermitted = false;
    try {
      const result = await request(permission);
      return handlePermission(result, t);
    } catch (error) {
      //handle error
      return isPermitted;
    }
